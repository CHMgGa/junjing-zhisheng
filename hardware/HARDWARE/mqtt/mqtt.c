...
//	printf("CONNECT _mqtt.txbuf=%s\n",(char*)_mqtt.txbuf);	// 调试txbuf内容
//	printf("CONNECT _mqtt.txlen=%d\n",_mqtt.txlen);	// 调试txbuf内容
	while(cnt--)
	{
		memset(_mqtt.rxbuf,0,_mqtt.rxlen);
		...
		while(wait--)
		{
			...
			printf("USART3_RX_BUF[0]=%x,USART3_RX_BUF[1]=%x\r\n",USART3_RX_BUF[0],USART3_RX_BUF[1]);
			printf("USART3_RX_BUF[2]=%x,USART3_RX_BUF[3]=%x\r\n",USART3_RX_BUF[2],USART3_RX_BUF[3]);
			printf("USART3_RX_BUF[4]=%x,USART3_RX_BUF[5]=%x\r\n",USART3_RX_BUF[4],USART3_RX_BUF[5]);
//			//CONNECT
...
{    
   ...
   u16 topiclen = strlen(topic);
		u8 cnt=2;
	u8 wait;
	u16 DataLen = 2 + (topiclen+2) + (whether?1:0);//可变报头的长度（2字节）加上有效载荷的长度
	_mqtt.txlen=0;
	//固定报头
	//控制报文类型
	memset(_mqtt.txbuf,0,_mqtt.txlen);
...
static u8 PublishData(char *topic, char *message, u8 qos)
{
    ...
    static u16 id=0;
		int DataLen;
		memset(_mqtt.txbuf,0,sizeof(txbuf));
...
//	printf("message=%s\n",message);	// 调试txbuf内容
//	printf("messageLength=%d\n",messageLength);	// 调试txbuf内容
//	printf("_mqtt.txbuf0=%s\n",(char*)_mqtt.txbuf+0);	// 调试txbuf内容
//	printf("_mqtt.txbuf1=%s\n",(char*)_mqtt.txbuf+1);	// 调试txbuf内容
//	printf("_mqtt.txbuf2=%s\n",(char*)_mqtt.txbuf+2);	// 调试txbuf内容
//	printf("_mqtt.txbuf3=%s\n",(char*)_mqtt.txbuf+3);	// 调试txbuf内容
//	printf("_mqtt.txbuf4=%s\n",(char*)_mqtt.txbuf+4);	// 调试txbuf内容
//	printf("_mqtt.txbuf5=%s\n",(char*)_mqtt.txbuf+5);	// 调试txbuf内容
//	printf("_mqtt.txbuf6=%s\n",(char*)_mqtt.txbuf+6);	// 调试txbuf内容
//	printf("_mqtt.txbuf7=%s\n",(char*)_mqtt.txbuf+7);	// 调试txbuf内容
//	printf("_mqtt.txbuf8=%s\n",(char*)_mqtt.txbuf+8);	// 调试txbuf内容
//	printf("_mqtt.txbuf9=%s\n",(char*)_mqtt.txbuf+9);	// 调试txbuf内容
//	printf("_mqtt.txbuf10=%s\n",(char*)_mqtt.txbuf+10);	// 调试txbuf内容
//	printf("_mqtt.txbuf11=%s\n",(char*)_mqtt.txbuf+11);	// 调试txbuf内容
//	printf("_mqtt.txbuf12=%s\n",(char*)_mqtt.txbuf+12);	// 调试txbuf内容
//	printf("_mqtt.txbuf13=%s\n",(char*)_mqtt.txbuf+13);	// 调试txbuf内容
//	printf("_mqtt.txbuf14=%s\n",(char*)_mqtt.txbuf+14);	// 调试txbuf内容
//	printf("_mqtt.txbuf15=%s\n",(char*)_mqtt.txbuf+15);	// 调试txbuf内容
//	printf("_mqtt.txbuf16=%s\n",(char*)_mqtt.txbuf+16);	// 调试txbuf内容
//	printf("_mqtt.txbuf17=%s\n",(char*)_mqtt.txbuf+17);	// 调试txbuf内容
//	printf("_mqtt.txbuf18=%s\n",(char*)_mqtt.txbuf+18);	// 调试txbuf内容
//	printf("_mqtt.txbuf136=%s\n",(char*)_mqtt.txbuf+136);	// 调试txbuf内容
	//printf("_mqtt.txlen=%d\n",_mqtt.txlen);	// 调试txbuf内容
    return _mqtt.txlen;
}
...
{
	#ifdef _DEBUG_MQTT
	//Usart1_SendBuf(buf+3,len);//
	#endif
	//usart2_TxData(buf);
	Usart3_SendBuf(buf,len);//向WIFI 发送命令
	//printf("send property =%d\n",len);
	//_net.SendString(buf); //向WIFI 发送命令
}	
